...
fruta_buscada = input("¿Qué fruta buscas? ")
encontrada = False
...
if encontrada:
    print("¡La fruta fue encontrada!")
else:
    print("La fruta no está en la lista")
```

**Explicación:**
- `encontrada = False` → variable "bandera" (flag) que empieza en falso, para llevar registro de si se encontró la fruta.
- `for fruta in frutas:` → recorre toda la lista comparando cada fruta.
- `if fruta == fruta_buscada:` → si hay coincidencia...
- `encontrada = True` → ...se cambia la bandera a verdadero (esto queda guardado aunque el for siga recorriendo el resto).
- `if encontrada:` / `else:` → después de terminar el for, se revisa el estado final de la bandera para decidir qué mensaje mostrar.